...
parser.add_argument('--afn_DA', type=str, choices=['none', 'HAFN', 'SAFN'], default='none')
parser.add_argument('--weight_afn', type=float, default=0.01)
parser.add_argument('--radius_hafn', type=float, default=1)
parser.add_argument('--radius_safn', type=float, default=0.3)
...
def validate(val_loader, verb_model, criterion, num_class, noun_model=None, val_labels=False):
	...
	for i, (val_data_all, val_label, val_id) in enumerate(val_loader):
		...
		with torch.no_grad():
			...
			# compute output
			_, _, _, _, _, attn_val_verb, out_val_verb, out_val_2_verb, pred_domain_val_verb, feat_val_verb, _ = verb_model(val_data, val_data, [0,0,0], 0, is_train=False, reverse=False)
	...
	with open("/home/fontana/Pre_extracted_features/RGB/ek_tsm/D1-D2_test.pkl", "a") as f:
