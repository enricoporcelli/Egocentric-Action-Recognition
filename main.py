...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
	...
	for i, ((source_data, source_label, source_id), (target_data, target_label, target_id)) in data_loader:
		...
		if args.pretrain_source:
			...
			_, out_source, out_source_2, _, _, _, _, _, _, _, _ = model(source_data, target_data, beta_new, mu, is_train=True, reverse=False)
		...
		#====== forward pass data ======#
		attn_source, out_source, out_source_2, pred_domain_source, feat_source, attn_target, out_target, out_target_2, pred_domain_target, feat_target, feat_afn = model(source_data, target_data, beta_new, mu, is_train=True, reverse=False)
		...
		if args.adv_DA != 'none' and args.use_target != 'none':
			...
			losses_a.update(loss_adversarial, pred_domain.size(0))
		...
		# (IV) AFN
		if args.afn_DA != 'none' and args.use_target != 'none':
	
			if args.afn_DA == 'HAFN':

				s_verb_cls_loss = get_cls_lossHAFN(out_verb, label_verb)
				s_verb_fc2_ring_loss = get_L2norm_loss_self_drivenHAFN(feat_afn)

				s_noun_cls_loss = get_cls_lossHAFN(out_noun, label_noun)
				

				loss += s_verb_cls_loss + s_verb_fc2_ring_loss
			if args.afn_DA == 'SAFN':
				s_verb_cls_loss = get_cls_lossSAFN(out_verb, label_verb)
				s_verb_fc2_ring_loss=get_L2norm_loss_self_drivenSAFN(feat_afn)
				
				s_noun_cls_loss = get_cls_lossSAFN(out_noun, label_noun)
...
def validate(val_loader, model, criterion, num_class, epoch, log, tensor_writer):
	...
	for i, (val_data, val_label, _) in enumerate(val_loader):
		...
		with torch.no_grad():
			...
			# compute output
			_, _, _, _, _, attn_val, out_val, out_val_2, pred_domain_val, feat_val, _ = model(val_data, val_data, [0]*len(args.beta), 0, is_train=False, reverse=False)
